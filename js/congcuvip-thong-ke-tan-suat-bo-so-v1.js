function parse_numbers_string(numbers_string) {
    ...
    for (i = 0; i < numbers.length; i++) {
        ...
        if (cur_number.length == 0) continue;
        if (isNaN(cur_number)) continue;
        number_val = parseInt(cur_number) % 100;
        if (number_val < 0 || number_val > 99) continue;
        if (select_num.indexOf(number_val)) select_num.push(number_val);
    }
    fade_period = 500;
    ...
    for (i = 0; i < 100; i++) {
        select_string = "tr#" + i;
        if (select_num.indexOf(i) == -1) $(select_string).fadeOut(fade_period);
